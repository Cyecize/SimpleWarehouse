CREATE DATABASE  IF NOT EXISTS `wh_sladuk_svqt` /*!40100 DEFAULT CHARACTER SET utf8 COLLATE utf8_unicode_ci */;
USE `wh_sladuk_svqt`;
-- MySQL dump 10.13  Distrib 5.7.17, for Win64 (x86_64)
--
-- Host: 127.0.0.1    Database: wh_sladuk_svqt
-- ------------------------------------------------------
...
--
-- Dumping events for database 'wh_sladuk_svqt'
--

--
-- Dumping routines for database 'wh_sladuk_svqt'
--
...
/*!50001 VIEW `expenses_users_joined` AS select `e`.`id` AS `id`,`e`.`date` AS `date`,`e`.`revenue_amount` AS `revenue_amount`,`e`.`is_revised` AS `is_revised`,`e`.`comment` AS `comment`,`e`.`user_id` AS `user_id`,`u`.`username` AS `username` from (`expenses` `e` join `users` `u` on((`u`.`id` = `e`.`user_id`))) */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `invoices_users_joined` AS select `i`.`id` AS `id`,`i`.`date` AS `date`,`i`.`revenue_amount` AS `revenue_amount`,`i`.`comment` AS `comment`,`i`.`user_id` AS `user_id`,`u`.`username` AS `username` from (`invoices` `i` join `users` `u` on((`u`.`id` = `i`.`user_id`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `revenues_users_joined` AS select `e`.`id` AS `id`,`e`.`date` AS `date`,`e`.`revenue_amount` AS `revenue_amount`,`e`.`is_revised` AS `is_revised`,`e`.`comment` AS `comment`,`e`.`user_id` AS `user_id`,`u`.`username` AS `username` from (`revenues` `e` join `users` `u` on((`u`.`id` = `e`.`user_id`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `user_auth_joined` AS select `u`.`id` AS `id`,`u`.`auth_id` AS `auth_id`,`u`.`date_registered` AS `date_registered`,`u`.`is_enabled` AS `is_enabled`,`u`.`password` AS `password`,`u`.`username` AS `username`,`a`.`auth_type` AS `auth_type` from (`users` `u` join `authentications` `a` on((`a`.`id` = `u`.`auth_id`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2018-06-06 23:15:30
